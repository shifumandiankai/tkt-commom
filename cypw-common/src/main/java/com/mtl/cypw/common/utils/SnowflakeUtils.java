...
public class SnowflakeUtils {
    ...
    public synchronized static long nextId(){
        ...
        if (now < LAST_TIME_STAMP) {
            throw new RuntimeException(String.format("系统时间错误！ %d 毫秒内拒绝生成分布式ID！", START_TIME - now));
        }
        if (now == LAST_TIME_STAMP) {
    ...
    /**
     * 获取字符串 s 的字节数组，然后将数组的元素相加，对（max+1）取余
     */
    ...
    /**
     * 根据 host address 取余，发生异常就获取 0 到 31 之间的随机数
     */
    public static int getWorkId(){
        try {
            return getHostId(CommonUtil.getLocalIP(), WORK_MAX_NUM);
        } catch (Exception e) {
    ...
    /**
     * 根据 host name 取余，发生异常就获取 0 到 31 之间的随机数
     */
    public static int getDataId() {
        try {
            return getHostId(CommonUtil.getLocalIP(), DATA_MAX_NUM);
        } catch (Exception e) {
